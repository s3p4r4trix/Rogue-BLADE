...
3. **Strike Velocity Gating:** Drones must reach **40% of topSpeed** (`MIN_STRIKE_SPEED`) before strikes connect. Post-strike deflection (tangential fly-by) creates fluid attack patterns. 1.0s cooldown between strikes.
...
* **Strict Mechanics Adherence:** NEVER modify or add formulas that are not defined in `docs/core_mechanics.md`. Any logic refinements or new mechanics (e.g., Glancing Blows, Impulse-based Collisions) MUST be documented in `core_mechanics.md` and `game_design.md` BEFORE or concurrent with their implementation to maintain a single source of truth.
